...
Read the complete source before editing. Run `scripts/scaffold-markdown.mjs <file.md|-> --json` for a deterministic inventory of headings, fences, lists, pipe runs, and recognised tokens. The scaffold does not classify Preserve/Restructure/Sanitise/Compose/Unresolved and must not be treated as a rewrite plan.
